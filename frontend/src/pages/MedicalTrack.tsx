...
interface MedicalTrackProps {
    setStage: (stage: any) => void;
    setLaunchParams?: (params: any) => void;
}

export const MedicalTrack: React.FC<MedicalTrackProps> = ({ setStage, setLaunchParams }) => {
    const { user } = useAuth();
    const [selectedSpecialty, setSelectedSpecialty] = useState<string>('emergency');
    const [riskScore, setRiskScore] = useState<number | null>(null);
    const [isLoadingRisk, setIsLoadingRisk] = useState(true);
    const [currentPage, setCurrentPage] = useState(1);
    const drillsPerPage = 2;

    const specialties = [
        { id: 'emergency', label: 'Emergency Medicine', icon: Activity },
        { id: 'radiology', label: 'Radiology', icon: Target },
        { id: 'cardiology', label: 'Cardiology', icon: Activity },
        { id: 'surgery', label: 'Surgery', icon: Play },
        { id: 'psychiatry', label: 'Psychiatry', icon: BookOpen },
        { id: 'soap', label: 'SOAP Notes', icon: Clock },
        { id: 'discharge', label: 'Discharge Summaries', icon: ArrowRight },
    ];
    ...
    useEffect(() => {
        const fetchRisk = async () => {
            ...
            try {
                ...
                const response = await fetch(`${url}/api/medical/risk/${user.id}`, {
    ...
    const activeDrills = getMedicalDrillsBySpecialty(selectedSpecialty);
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-black uppercase text-text-main flex items-center gap-3">
                        ...
                        Medical Competency Engine
                    </h1>
                    <p className="text-text-muted mt-2 tracking-wide">Master specialized healthcare terminology and clinical documentation</p>
                </div>
