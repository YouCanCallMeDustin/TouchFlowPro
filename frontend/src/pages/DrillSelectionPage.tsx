...
interface DrillSelectionPageProps {
    lesson: Lesson;
    onStartDrill: (lesson: Lesson, drillText: string, variationId: string) => void;
    onBack: () => void;
}
        ...
        <div className="max-w-6xl mx-auto w-full px-4 py-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {lesson.practiceVariations?.map((drill, idx) => {
                    const variationId = `${lesson.id}-v${idx}`;
                    const isCompleted = completedDrills.has(variationId);
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-between mb-6">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-transform group-hover:scale-110 ${isCompleted ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-black/40 text-primary border-white/5'}`}>
                                            {isCompleted ? <CheckCircle size={18} /> : <Compass size={18} />}
                                        </div>
                                        <span className="text-[11px] font-black text-text-muted uppercase tracking-[0.4em]">Drill #{idx + 1}</span>
                                    </div>
                                    <div className={`text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full border ${isCompleted ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-primary/40 bg-primary/5 border-primary/10'}`}>
                                ...
                                <Button
                                    onClick={() => onStartDrill(lesson, drill, variationId)}
                                ...
                                >
                                    {isCompleted ? 'Practice Again' : 'Initiate Drill'}
                                    <ArrowRight size={14} />
                                </Button>
                            </div>
