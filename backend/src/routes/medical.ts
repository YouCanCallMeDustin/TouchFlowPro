...
export async function calculateTerminologyRisk(userId: string): Promise<number> {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

    const weakTerms = await prisma.medicalWeakTerm.findMany({
        where: {
            userId,
            mistakeCount: { gte: 3 },
...
router.get('/risk/:userId', async (req, res) => {
    try {
        ...
        const score = await calculateTerminologyRisk(userId);
        res.json({ score });
    } catch (error) {
        console.error('Error fetching medical risk:', error);
