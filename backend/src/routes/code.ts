...
export async function calculateCodeSyntaxRisk(userId: string): Promise<number> {
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

    const weakTerms = await prisma.codeWeakTerm.findMany({
        where: {
            userId,
            mistakeCount: { gte: 3 },
...
router.get('/risk/:userId', async (req, res) => {
    try {
        ...
        const score = await calculateCodeSyntaxRisk(userId);
        res.json({ score });
    } catch (error) {
        console.error('Error fetching code risk:', error);
