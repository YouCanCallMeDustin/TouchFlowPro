...
import { generatePersonalizedDrill, calculateTroubleKeyDensity } from '../utils/drillGenerator';
...
router.get('/:userId/level', async (req, res) => {
    try {
        ...
        // Get user stats
        const [drillResults, userProgress] = await Promise.all([
            prisma.drillResult.findMany({
                where: { userId },
                orderBy: { timestamp: 'desc' },
                take: 20
        ...
        // Calculate level
        const recentWPM = drillResults.map(r => r.netWPM);
        const recentAccuracy = drillResults.map(r => r.accuracy);
