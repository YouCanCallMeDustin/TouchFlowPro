                        ...
                        <div className="flex bg-slate-500/5 dark:bg-white/5 p-1.5 rounded-2xl border border-slate-200/50 dark:border-white/5 backdrop-blur-sm">
                            {[7, 30, 90].map((d) => (
                                <button
                                    key={d}
                                    onClick={() => setDays(d)}
                                    className={`px-6 py-2 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] transition-all ${days === d ? 'bg-primary text-white shadow-lg shadow-primary/20 scale-105' : 'text-text-muted hover:text-text-main'}`}
                                >
                                    {d} Days
                                </button>
                            ))}
